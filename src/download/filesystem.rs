...
///
/// - `segment_map` present **or** `transfer_format_version >= 1`: map-authoritative.
///   `downloaded_bytes = sum(written)` when a map exists; **never** `metadata_len`.
///   Version ≥ 1 with no map: leave `downloaded_bytes` unchanged (safe no-op).
...
pub async fn reconcile_partial_progress(job: &mut Job) -> ReconcileResult {
    ...
    if job.segment_map.is_some() || version_gated {
        if let Some(map) = job.segment_map.as_ref() {
            // Consistency is checked for later Resume; progress is always map-sum.
            let _ = map.is_consistent();
