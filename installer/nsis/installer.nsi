; RusticDL NSIS template for cargo-packager.
; Based on cargo-packager 0.11.x default installer.nsi, with post-install /
; pre-uninstall hooks that register the browser native messaging host.
;
...
; Prefer the product display name over the long package description so
; Task Manager / Explorer show "RusticDL" (not a marketing sentence).
...
Section Install
  ...
  ${Else}
    DetailPrint "RusticDL Backend registered successfully."
  ${EndIf}
...
Section Uninstall
  ...
  ${Else}
    DetailPrint "RusticDL Backend unregistered."
  ${EndIf}
  ...
  ; Runtime-generated native messaging manifests (created by register script at install time)
  RMDir /r "$INSTDIR\native-messaging"
